  ...
  const fetchData = async () => {
    if (user) {
      try {
        ...
        const response = await fetch(`/api/section?username=${username}&board=${board}&title=${section}`)
        if (!response.ok) {
  ...
  return (
    <div className='flex flex-col items-center mt-10 gap-5'>
      <div className='flex flex-col gap-2 items-center'>
        <h2 className="text-2xl font-semibold text-muted-foreground">{data.board?.title}</h2>
      ...
      <div className="flex">
        {data.pins && <Reorganize _id={data._id} pins={data?.pins} setPins={setReorderedPins} onReorder={handleReorder} />}
