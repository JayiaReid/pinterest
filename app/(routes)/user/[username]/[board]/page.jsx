    ...
    if (user) {
      ...
      try {
        const response = await fetch(
          `/api/board?username=${username}&title=${board}`
        );
        if (!response.ok) {
    ...
    <div className="flex flex-col items-center mt-10 gap-5">
      ...
      <div className="flex">
        <Button
          ...
          onClick={() =>
            router.push(
              `/user/${username}/${board}/more_ideas?board_id=${data._id}`
            )
          }
          className=" text-sm text-foreground p-5 rounded-3xl shadow-none flex flex-col gap-1"
        >
          <Sparkles
            className="bg-muted hover:bg-[#E2E2E2] duration-300 p-6 rounded-2xl"
            size={80}
            strokeWidth={2.5}
          />{" "}
