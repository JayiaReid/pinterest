...
const page = () => {
  ...
  return (
    <div className="p-5">
        ...
        <DropdownMenu onOpenChange={(isOpen) => !isOpen && setActive(false)}>
          ...
          <DropdownMenuContent className="rounded-2xl m-5 p-2">
            ...
            <DropdownMenuItem
              ...
              className="rounded-xl flex justify-between p-2 w-56"
            >
      ...
      {filled ? (
        <div className=" grid grid-cols-1 xs:grid-cols-1 xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2">
          {getSortedBoards().map((board, index) => (
            <Board key={index} board={board} user={username} />
          ))}
        </div>
      ) : (
        <div className="p-5 grid grid-cols-2 xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2">
